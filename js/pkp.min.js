...
v):m.addEventListener("load",v,!1);setTimeout(c,15);try{i.submit()}catch(x){document.createElement("form").submit.apply(i)}}finally{i.setAttribute("action",e),i.setAttribute("enctype",B),b?i.setAttribute("target",b):q.removeAttr("target"),a(h).remove()}}function v(c){if(!r.aborted&&!P)if(x=f(m),x||(d("cannot access response document"),c=I),c===O&&r)r.abort("timeout"),B.reject(r,"timeout");else if(c==I&&r)r.abort("server abort"),B.reject(r,"error","server abort");else if(x&&x.location.href!=l.iframeSrc||
t){m.detachEvent?m.detachEvent("onload",v):m.removeEventListener("load",v,!1);var c="success",b;try{if(t)throw"timeout";var e="xml"==l.dataType||x.XMLDocument||a.isXMLDoc(x);d("isXml="+e);if(!e&&(window.opera&&(null===x.body||!x.body.innerHTML))&&--T){d("requeing onLoad callback, DOM not available");setTimeout(v,250);return}var g=x.body?x.body:x.documentElement;r.responseText=g?g.innerHTML:null;r.responseXML=x.XMLDocument?x.XMLDocument:x;e&&(l.dataType="xml");r.getResponseHeader=function(a){return{"content-type":l.dataType}[a.toLowerCase()]};
...
a.globalEval(s));C=s}catch(y){c="parsererror",r.error=b=y||c}}catch(z){d("error caught: ",z),c="error",r.error=b=z||c}r.aborted&&(d("upload aborted"),c=null);r.status&&(c=200<=r.status&&300>r.status||304===r.status?"success":"error");"success"===c?(l.success&&l.success.call(l.context,C,"success",r),B.resolve(r.responseText,"success",r),A&&a.event.trigger("ajaxSuccess",[r,l])):c&&(void 0===b&&(b=r.statusText),l.error&&l.error.call(l.context,r,c,b),B.reject(r,"error",b),A&&a.event.trigger("ajaxError",
[r,l,b]));A&&a.event.trigger("ajaxComplete",[r,l]);A&&!--a.active&&a.event.trigger("ajaxStop");l.complete&&l.complete.call(l.context,r,c);P=!0;l.timeout&&clearTimeout(G);setTimeout(function(){l.iframeTarget?p.attr("src",l.iframeSrc):p.remove();r.responseXML=null},100)}}var i=q[0],k,l,A,w,p,m,r,t,G,B=a.Deferred();B.abort=function(a){r.abort(a)};if(e)for(k=0;k<s.length;k++)e=a(s[k]),h?e.prop("disabled",!1):e.removeAttr("disabled");l=a.extend(!0,{},a.ajaxSettings,c);l.context=l.context||l;w="jqFormIO"+
...
m.contentWindow.document.execCommand("Stop")}catch(e){}p.attr("src",l.iframeSrc);r.error=b;l.error&&l.error.call(l.context,r,b,c);A&&a.event.trigger("ajaxError",[r,l,b]);l.complete&&l.complete.call(l.context,r,b)}};(A=l.global)&&0===a.active++&&a.event.trigger("ajaxStart");A&&a.event.trigger("ajaxSend",[r,l]);if(l.beforeSend&&!1===l.beforeSend.call(l.context,r,l))return l.global&&a.active--,B.reject(),B;if(r.aborted)return B.reject(),B;if(e=i.clk)if((k=e.name)&&!e.disabled)l.extraData=l.extraData||
{},l.extraData[k]=e.value,"image"==e.type&&(l.extraData[k+".x"]=i.clk_x,l.extraData[k+".y"]=i.clk_y);var O=1,I=2,e=a("meta[name=csrf-token]").attr("content");if((k=a("meta[name=csrf-param]").attr("content"))&&e)l.extraData=l.extraData||{},l.extraData[k]=e;l.forceSync?g():setTimeout(g,10);var C,x,T=50,P,u=a.parseXML||function(a,c){window.ActiveXObject?(c=new ActiveXObject("Microsoft.XMLDOM"),c.async="false",c.loadXML(a)):c=(new DOMParser).parseFromString(a,"text/xml");return c&&c.documentElement&&
"parsererror"!=c.documentElement.nodeName?c:null},E=a.parseJSON||function(a){return window.eval("("+a+")")};return B}if(!this.length)return d("ajaxSubmit: skipping submit process - no element selected"),this;var n,k,q=this;"function"==typeof c?c={success:c}:void 0===c&&(c={});n=c.type||this.attr2("method");k=c.url||this.attr2("action");(k=(k="string"===typeof k?a.trim(k):"")||window.location.href||"")&&(k=(k.match(/^([^#]+)/)||[])[1]);c=a.extend(!0,{url:k,success:a.ajaxSettings.success,type:n||a.ajaxSettings.type,
iframeSrc:/^https/i.test(window.location.href||"")?"javascript:false":"about:blank"},c);k={};this.trigger("form-pre-serialize",[this,c,k]);if(k.veto)return d("ajaxSubmit: submit vetoed via form-pre-serialize trigger"),this;if(c.beforeSerialize&&!1===c.beforeSerialize(this,c))return d("ajaxSubmit: submit aborted via beforeSerialize callback"),this;var p=c.traditional;void 0===p&&(p=a.ajaxSettings.traditional);var s=[],m,u=this.formToArray(c.semantic,s);c.data&&(c.extraData=c.data,m=a.param(c.data,
p));if(c.beforeSubmit&&!1===c.beforeSubmit(u,this,c))return d("ajaxSubmit: submit aborted via beforeSubmit callback"),this;this.trigger("form-submit-validate",[u,this,c,k]);if(k.veto)return d("ajaxSubmit: submit vetoed via form-submit-validate trigger"),this;k=a.param(u,p);m&&(k=k?k+"&"+m:m);"GET"==c.type.toUpperCase()?(c.url+=(0<=c.url.indexOf("?")?"&":"?")+k,c.data=null):c.data=k;var t=[];c.resetForm&&t.push(function(){q.resetForm()});c.clearForm&&t.push(function(){q.clearForm(c.includeHidden)});
if(!c.dataType&&c.target){var g=c.success||function(){};t.push(function(b){var d=c.replaceTarget?"replaceWith":"html";a(c.target)[d](b).each(g,arguments)})}else c.success&&t.push(c.success);c.success=function(a,b,d){for(var e=c.context||this,f=0,g=t.length;f<g;f++)t[f].apply(e,[a,b,d||q,q])};if(c.error){var D=c.error;c.error=function(a,b,d){D.apply(c.context||this,[a,b,d,q])}}if(c.complete){var v=c.complete;c.complete=function(a,b){v.apply(c.context||this,[a,b,q])}}m=0<a("input[type=file]:enabled",
this).filter(function(){return""!==a(this).val()}).length;k="multipart/form-data"==q.attr("enctype")||"multipart/form-data"==q.attr("encoding");p=e&&f;d("fileAPI :"+p);var l;if(!1!==c.iframe&&(c.iframe||(m||k)&&!p))c.closeKeepAlive?a.get(c.closeKeepAlive,function(){l=b(u)}):l=b(u);else if((m||k)&&p){var A=new FormData;for(k=0;k<u.length;k++)A.append(u[k].name,u[k].value);if(c.extraData){k=a.param(c.extraData,c.traditional).split("&");p=k.length;m=[];var w,E;for(w=0;w<p;w++)k[w]=k[w].replace(/\+/g,
" "),E=k[w].split("="),m.push([decodeURIComponent(E[0]),decodeURIComponent(E[1])]);for(k=0;k<m.length;k++)m[k]&&A.append(m[k][0],m[k][1])}c.data=null;m=a.extend(!0,{},a.ajaxSettings,c,{contentType:!1,processData:!1,cache:!1,type:n||"POST"});c.uploadProgress&&(m.xhr=function(){var b=a.ajaxSettings.xhr();b.upload&&b.upload.addEventListener("progress",function(a){var b=0,d=a.loaded||a.position,e=a.total;a.lengthComputable&&(b=Math.ceil(100*(d/e)));c.uploadProgress(a,d,e,b)},!1);return b});m.data=null;
var z=m.beforeSend;m.beforeSend=function(a,b){b.data=c.formData?c.formData:A;z&&z.call(this,a,b)};l=a.ajax(m)}else l=a.ajax(c);q.removeData("jqxhr").data("jqxhr",l);for(m=0;m<s.length;m++)s[m]=null;this.trigger("form-submit-notify",[this,c]);return this};a.fn.ajaxForm=function(e){e=e||{};e.delegation=e.delegation&&a.isFunction(a.fn.on);if(!e.delegation&&0===this.length){var f=this.selector,h=this.context;if(!a.isReady&&f)return d("DOM not ready, queuing ajaxForm"),a(function(){a(f,h).ajaxForm(e)}),
...
b){var d=[];if(0===this.length)return d;var f=this[0],h=this.attr("id"),p=c?f.getElementsByTagName("*"):f.elements;p&&!/MSIE [678]/.test(navigator.userAgent)&&(p=a(p).get());h&&(h=a(':input[form="'+h+'"]').get(),h.length&&(p=(p||[]).concat(h)));if(!p||!p.length)return d;var s,m,u,t,g;s=0;for(g=p.length;s<g;s++)if(t=p[s],(h=t.name)&&!t.disabled)if(c&&f.clk&&"image"==t.type)f.clk==t&&(d.push({name:h,value:a(t).val(),type:t.type}),d.push({name:h+".x",value:f.clk_x},{name:h+".y",value:f.clk_y}));else if((u=
a.fieldValue(t,!0))&&u.constructor==Array){b&&b.push(t);m=0;for(t=u.length;m<t;m++)d.push({name:h,value:u[m]})}else if(e&&"file"==t.type)if(b&&b.push(t),u=t.files,u.length)for(m=0;m<u.length;m++)d.push({name:h,value:u[m],type:t.type});else d.push({name:h,value:"",type:t.type});else null!==u&&"undefined"!=typeof u&&(b&&b.push(t),d.push({name:h,value:u,type:t.type,required:t.required}));if(!c&&f.clk&&(p=a(f.clk),s=p[0],(h=s.name)&&!s.disabled&&"image"==s.type))d.push({name:h,value:p.val()}),d.push({name:h+
...
else c=a('<li class="selectBox-optgroup" />'),c.text(a(this).attr("label")),b.append(c),b=e(a(this),b)});return b};switch(b){case "inline":return d=a('<ul class="selectBox-options" />'),d=e(c,d),d.find("A").bind("mouseover.selectBox",function(){s(c,a(this).parent())}).bind("mouseout.selectBox",function(){m(c,a(this).parent())}).bind("mousedown.selectBox",function(a){a.preventDefault();c.selectBox("control").hasClass("selectBox-active")||c.selectBox("control").focus()}).bind("mouseup.selectBox",function(b){q();
p(c,a(this).parent(),b)}),D(d),d;case "dropdown":d=a('<ul class="selectBox-dropdown-menu selectBox-options" />');d=e(c,d);d.data("selectBox-select",c).css("display","none").appendTo("BODY").find("A").bind("mousedown.selectBox",function(a){a.preventDefault();a.screenX===d.data("selectBox-down-at-x")&&a.screenY===d.data("selectBox-down-at-y")&&(d.removeData("selectBox-down-at-x").removeData("selectBox-down-at-y"),q())}).bind("mouseup.selectBox",function(b){b.screenX===d.data("selectBox-down-at-x")&&
b.screenY===d.data("selectBox-down-at-y")||(d.removeData("selectBox-down-at-x").removeData("selectBox-down-at-y"),p(c,a(this).parent()),q())}).bind("mouseover.selectBox",function(){s(c,a(this).parent())}).bind("mouseout.selectBox",function(){m(c,a(this).parent())});var f=c.attr("class")||"";if(""!==f){var f=f.split(" "),g;for(g in f)d.addClass(f[g]+"-selectBox-dropdown-menu")}D(d);return d}},j=function(c){return("selectBox-label "+(a(c).find("OPTION:selected").attr("class")||"")).replace(/\s+$/,"")},
i=function(c){return a(c).find("OPTION:selected").text()||"\u00a0"},n=function(c){var c=a(c),b=c.data("selectBox-control");b&&b.find(".selectBox-label").attr("class",j(c)).text(i(c))},k=function(c){var c=a(c),b=c.data("selectBox-control"),d=c.data("selectBox-settings"),e=b.data("selectBox-options");if(b.hasClass("selectBox-disabled"))return!1;q();var f=isNaN(b.css("borderBottomWidth"))?0:parseInt(b.css("borderBottomWidth"));e.width(b.innerWidth()).css({top:b.offset().top+b.outerHeight()-f,left:b.offset().left});
if(c.triggerHandler("beforeopen"))return!1;f=function(){c.triggerHandler("open",{_selectBox:!0})};switch(d.menuTransition){case "fade":e.fadeIn(d.menuSpeed,f);break;case "slide":e.slideDown(d.menuSpeed,f);break;default:e.show(d.menuSpeed,f)}d.menuSpeed||f();d=e.find(".selectBox-selected:first");u(c,d,!0);s(c,d);b.addClass("selectBox-menuShowing");a(document).bind("mousedown.selectBox",function(c){a(c.target).parents().andSelf().hasClass("selectBox-options")||q()})},q=function(){0!==a(".selectBox-dropdown-menu:visible").length&&
...
"inline",d=h(c,f),e.data("selectBox-options",d),f){case "inline":e.append(d);break;case "dropdown":n(c),a("BODY").append(d)}});break;case "value":if(void 0===b)return a(this).val();a(this).each(function(){var c,d=b;c=a(this);c.val(d);d=c.val();null===d&&(d=c.children().first().val(),c.val(d));var e=c.data("selectBox-control");if(e){var f=c.data("selectBox-settings"),e=e.data("selectBox-options");n(c);e.find(".selectBox-selected").removeClass("selectBox-selected");e.find("A").each(function(){if("object"===
...
"select"===b.tagName.toLowerCase()&&(b=a(b),!b.data("selectBox-control"))){var e=a('<a class="selectBox" />'),f=b.attr("multiple")||1<parseInt(b.attr("size")),n=c||{};e.width(b.outerWidth()).addClass(b.attr("class")).attr("title",b.attr("title")||"").attr("tabindex",parseInt(b.attr("tabindex"))).css("display","inline-block").bind("focus.selectBox",function(){this!==document.activeElement&&document.body!==document.activeElement&&a(document.activeElement).blur();e.hasClass("selectBox-active")||(e.addClass("selectBox-active"),
b.trigger("focus"))}).bind("blur.selectBox",function(){e.hasClass("selectBox-active")&&(e.removeClass("selectBox-active"),b.trigger("blur"))});a(window).data("selectBox-bindings")||a(window).data("selectBox-bindings",!0).bind("scroll.selectBox",q).bind("resize.selectBox",q);b.attr("disabled")&&e.addClass("selectBox-disabled");b.bind("click.selectBox",function(a){e.focus();a.preventDefault()});if(f){if(d=h(b,"inline"),e.append(d).data("selectBox-options",d).addClass("selectBox-inline selectBox-menuShowing").bind("keydown.selectBox",
function(a){t(b,a)}).bind("keypress.selectBox",function(a){g(b,a)}).bind("mousedown.selectBox",function(c){a(c.target).is("A.selectBox-inline")&&c.preventDefault();e.hasClass("selectBox-focus")||e.focus()}).insertAfter(b),!b[0].style.height){var f=b.attr("size")?parseInt(b.attr("size")):5,p=e.clone().removeAttr("id").css({position:"absolute",top:"-9999em"}).show().appendTo("body");p.find(".selectBox-options").html("<li><a>\u00a0</a></li>");var m=parseInt(p.find(".selectBox-options A:first").html("&nbsp;").outerHeight());
p.remove();e.height(m*f)}}else f=a('<span class="selectBox-label" />'),p=a('<span class="selectBox-arrow" />'),f.attr("class",j(b)).text(i(b)),d=h(b,"dropdown"),d.appendTo("BODY"),e.data("selectBox-options",d).addClass("selectBox-dropdown").append(f).append(p).bind("mousedown.selectBox",function(a){e.hasClass("selectBox-menuShowing")?q():(a.stopPropagation(),d.data("selectBox-down-at-x",a.screenX).data("selectBox-down-at-y",a.screenY),k(b))}).bind("keydown.selectBox",function(a){t(b,a)}).bind("keypress.selectBox",
function(a){g(b,a)}).bind("open.selectBox",function(a,c){c&&!0===c._selectBox||k(b)}).bind("close.selectBox",function(a,c){c&&!0===c._selectBox||q()}).insertAfter(b),p=e.width()-p.outerWidth()-parseInt(f.css("paddingLeft"))-parseInt(f.css("paddingLeft")),f.width(p);D(e);b.addClass("selectBox").data("selectBox-control",e).data("selectBox-settings",n).hide()}})}return a(this)}})}(jQuery);(function(){function a(a){return function(c,b,d,e){var b=D(b,e,4),f=!y(c)&&g.keys(c),h=(f||c).length,i=0<a?0:h-1;3>arguments.length&&(d=c[f?f[i]:i],i+=a);for(var k=b,j=d;0<=i&&i<h;i+=a)var n=f?f[i]:i,j=k(j,c[n],n,c);return j}}function c(a){return function(c,b,d){for(var b=v(b,d),d=z(c),e=0<a?0:d-1;0<=e&&e<d;e+=a)if(b(c[e],e,c))return e;return-1}}function b(a,c,b){return function(d,e,f){var h=0,i=z(d);if("number"==typeof f)0<a?h=0<=f?f:Math.max(f+i,h):i=0<=f?Math.min(f+1,i):f+i+1;else if(b&&f&&i)return f=
b(d,e),d[f]===e?f:-1;if(e!==e)return f=c(n.call(d,h,i),g.isNaN),0<=f?f+h:-1;for(f=0<a?h:i-1;0<=f&&f<i;f+=a)if(d[f]===e)return f;return-1}}function d(a,c){var b=Q.length,d=a.constructor,d=g.isFunction(d)&&d.prototype||j,e="constructor";for(g.has(a,e)&&!g.contains(c,e)&&c.push(e);b--;)e=Q[b],e in a&&(a[e]!==d[e]&&!g.contains(c,e))&&c.push(e)}var e=this,f=e._,h=Array.prototype,j=Object.prototype,i=h.push,n=h.slice,k=j.toString,q=j.hasOwnProperty,p=Array.isArray,s=Object.keys,m=Function.prototype.bind,
...
d,e,f){return a.call(c,b,d,e,f)}}return function(){return a.apply(c,arguments)}},v=function(a,c,b){return null==a?g.identity:g.isFunction(a)?D(a,c,b):g.isObject(a)?g.matcher(a):g.property(a)};g.iteratee=function(a,c){return v(a,c,Infinity)};var l=function(a,c){return function(b){var d=arguments.length;if(2>d||null==b)return b;for(var e=1;e<d;e++)for(var f=arguments[e],g=a(f),h=g.length,i=0;i<h;i++){var k=g[i];if(!c||void 0===b[k])b[k]=f[k]}return b}},A=function(a){if(!g.isObject(a))return{};if(u)return u(a);
t.prototype=a;a=new t;t.prototype=null;return a},w=function(a){return function(c){return null==c?void 0:c[a]}},E=Math.pow(2,53)-1,z=w("length"),y=function(a){a=z(a);return"number"==typeof a&&0<=a&&a<=E};g.each=g.forEach=function(a,c,b){var c=D(c,b),d;if(y(a)){b=0;for(d=a.length;b<d;b++)c(a[b],b,a)}else{var e=g.keys(a),b=0;for(d=e.length;b<d;b++)c(a[e[b]],e[b],a)}return a};g.map=g.collect=function(a,c,b){for(var c=v(c,b),b=!y(a)&&g.keys(a),d=(b||a).length,e=Array(d),f=0;f<d;f++){var h=b?b[f]:f;e[f]=
c(a[h],h,a)}return e};g.reduce=g.foldl=g.inject=a(1);g.reduceRight=g.foldr=a(-1);g.find=g.detect=function(a,c,b){c=y(a)?g.findIndex(a,c,b):g.findKey(a,c,b);if(void 0!==c&&-1!==c)return a[c]};g.filter=g.select=function(a,c,b){var d=[],c=v(c,b);g.each(a,function(a,b,e){c(a,b,e)&&d.push(a)});return d};g.reject=function(a,c,b){return g.filter(a,g.negate(v(c)),b)};g.every=g.all=function(a,c,b){for(var c=v(c,b),b=!y(a)&&g.keys(a),d=(b||a).length,e=0;e<d;e++){var f=b?b[e]:e;if(!c(a[f],f,a))return!1}return!0};
g.some=g.any=function(a,c,b){for(var c=v(c,b),b=!y(a)&&g.keys(a),d=(b||a).length,e=0;e<d;e++){var f=b?b[e]:e;if(c(a[f],f,a))return!0}return!1};g.contains=g.includes=g.include=function(a,c,b,d){y(a)||(a=g.values(a));if("number"!=typeof b||d)b=0;return 0<=g.indexOf(a,c,b)};g.invoke=function(a,c){var b=n.call(arguments,2),d=g.isFunction(c);return g.map(a,function(a){var e=d?c:a[c];return null==e?e:e.apply(a,b)})};g.pluck=function(a,c){return g.map(a,g.property(c))};g.where=function(a,c){return g.filter(a,
g.matcher(c))};g.findWhere=function(a,c){return g.find(a,g.matcher(c))};g.max=function(a,c,b){var d=-Infinity,e=-Infinity,f;if(null==c&&null!=a)for(var a=y(a)?a:g.values(a),h=0,i=a.length;h<i;h++)b=a[h],b>d&&(d=b);else c=v(c,b),g.each(a,function(a,b,g){f=c(a,b,g);if(f>e||-Infinity===f&&-Infinity===d)d=a,e=f});return d};g.min=function(a,c,b){var d=Infinity,e=Infinity,f;if(null==c&&null!=a)for(var a=y(a)?a:g.values(a),h=0,i=a.length;h<i;h++)b=a[h],b<d&&(d=b);else c=v(c,b),g.each(a,function(a,b,g){f=
c(a,b,g);if(f<e||Infinity===f&&Infinity===d)d=a,e=f});return d};g.shuffle=function(a){for(var a=y(a)?a:g.values(a),c=a.length,b=Array(c),d=0,e;d<c;d++)e=g.random(0,d),e!==d&&(b[d]=b[e]),b[e]=a[d];return b};g.sample=function(a,c,b){return null==c||b?(y(a)||(a=g.values(a)),a[g.random(a.length-1)]):g.shuffle(a).slice(0,Math.max(0,c))};g.sortBy=function(a,c,b){c=v(c,b);return g.pluck(g.map(a,function(a,b,d){return{value:a,index:b,criteria:c(a,b,d)}}).sort(function(a,c){var b=a.criteria,d=c.criteria;if(b!==
d){if(b>d||void 0===b)return 1;if(b<d||void 0===d)return-1}return a.index-c.index}),"value")};var H=function(a){return function(c,b,d){var e={},b=v(b,d);g.each(c,function(d,f){var g=b(d,f,c);a(e,d,g)});return e}};g.groupBy=H(function(a,c,b){g.has(a,b)?a[b].push(c):a[b]=[c]});g.indexBy=H(function(a,c,b){a[b]=c});g.countBy=H(function(a,c,b){g.has(a,b)?a[b]++:a[b]=1});g.toArray=function(a){return!a?[]:g.isArray(a)?n.call(a):y(a)?g.map(a,g.identity):g.values(a)};g.size=function(a){return null==a?0:y(a)?
a.length:g.keys(a).length};g.partition=function(a,c,b){var c=v(c,b),d=[],e=[];g.each(a,function(a,b,f){(c(a,b,f)?d:e).push(a)});return[d,e]};g.first=g.head=g.take=function(a,c,b){return null==a?void 0:null==c||b?a[0]:g.initial(a,a.length-c)};g.initial=function(a,c,b){return n.call(a,0,Math.max(0,a.length-(null==c||b?1:c)))};g.last=function(a,c,b){return null==a?void 0:null==c||b?a[a.length-1]:g.rest(a,Math.max(0,a.length-c))};g.rest=g.tail=g.drop=function(a,c,b){return n.call(a,null==c||b?1:c)};g.compact=
function(a){return g.filter(a,g.identity)};var F=function(a,c,b,d){for(var e=[],f=0,d=d||0,h=z(a);d<h;d++){var i=a[d];if(y(i)&&(g.isArray(i)||g.isArguments(i))){c||(i=F(i,c,b));var k=0,j=i.length;for(e.length+=j;k<j;)e[f++]=i[k++]}else b||(e[f++]=i)}return e};g.flatten=function(a,c){return F(a,c,!1)};g.without=function(a){return g.difference(a,n.call(arguments,1))};g.uniq=g.unique=function(a,c,b,d){g.isBoolean(c)||(d=b,b=c,c=!1);null!=b&&(b=v(b,d));for(var d=[],e=[],f=0,h=z(a);f<h;f++){var i=a[f],
k=b?b(i,f,a):i;c?((!f||e!==k)&&d.push(i),e=k):b?g.contains(e,k)||(e.push(k),d.push(i)):g.contains(d,i)||d.push(i)}return d};g.union=function(){return g.uniq(F(arguments,!0,!0))};g.intersection=function(a){for(var c=[],b=arguments.length,d=0,e=z(a);d<e;d++){var f=a[d];if(!g.contains(c,f)){for(var h=1;h<b&&g.contains(arguments[h],f);h++);h===b&&c.push(f)}}return c};g.difference=function(a){var c=F(arguments,!0,!0,1);return g.filter(a,function(a){return!g.contains(c,a)})};g.zip=function(){return g.unzip(arguments)};
g.unzip=function(a){for(var c=a&&g.max(a,z).length||0,b=Array(c),d=0;d<c;d++)b[d]=g.pluck(a,d);return b};g.object=function(a,c){for(var b={},d=0,e=z(a);d<e;d++)c?b[a[d]]=c[d]:b[a[d][0]]=a[d][1];return b};g.findIndex=c(1);g.findLastIndex=c(-1);g.sortedIndex=function(a,c,b,d){for(var b=v(b,d,1),c=b(c),d=0,e=z(a);d<e;){var f=Math.floor((d+e)/2);b(a[f])<c?d=f+1:e=f}return d};g.indexOf=b(1,g.findIndex,g.sortedIndex);g.lastIndexOf=b(-1,g.findLastIndex);g.range=function(a,c,b){null==c&&(c=a||0,a=0);for(var b=
b||1,c=Math.max(Math.ceil((c-a)/b),0),d=Array(c),e=0;e<c;e++,a+=b)d[e]=a;return d};var M=function(a,c,b,d,e){if(!(d instanceof c))return a.apply(b,e);c=A(a.prototype);a=a.apply(c,e);return g.isObject(a)?a:c};g.bind=function(a,c){if(m&&a.bind===m)return m.apply(a,n.call(arguments,1));if(!g.isFunction(a))throw new TypeError("Bind must be called on a function");var b=n.call(arguments,2),d=function(){return M(a,d,c,this,b.concat(n.call(arguments)))};return d};g.partial=function(a){var c=n.call(arguments,
...
{};return b};g.delay=function(a,c){var b=n.call(arguments,2);return setTimeout(function(){return a.apply(null,b)},c)};g.defer=g.partial(g.delay,g,1);g.throttle=function(a,c,b){var d,e,f,h=null,i=0;b||(b={});var k=function(){i=!1===b.leading?0:g.now();h=null;f=a.apply(d,e);h||(d=e=null)};return function(){var j=g.now();!i&&!1===b.leading&&(i=j);var n=c-(j-i);d=this;e=arguments;0>=n||n>c?(h&&(clearTimeout(h),h=null),i=j,f=a.apply(d,e),h||(d=e=null)):!h&&!1!==b.trailing&&(h=setTimeout(k,n));return f}};
...
b[d];return c};g.functions=g.methods=function(a){var c=[],b;for(b in a)g.isFunction(a[b])&&c.push(b);return c.sort()};g.extend=l(g.allKeys);g.extendOwn=g.assign=l(g.keys);g.findKey=function(a,c,b){for(var c=v(c,b),b=g.keys(a),d,e=0,f=b.length;e<f;e++)if(d=b[e],c(a[d],d,a))return d};g.pick=function(a,c,b){var d={},e=a,f,h;if(null==e)return d;g.isFunction(c)?(h=g.allKeys(e),f=D(c,b)):(h=F(arguments,!1,!1,1),f=function(a,c,b){return c in b},e=Object(e));for(var i=0,k=h.length;i<k;i++){var j=h[i],n=e[j];
f(n,j,e)&&(d[j]=n)}return d};g.omit=function(a,c,b){if(g.isFunction(c))c=g.negate(c);else var d=g.map(F(arguments,!1,!1,1),String),c=function(a,c){return!g.contains(d,c)};return g.pick(a,c,b)};g.defaults=l(g.allKeys,!0);g.create=function(a,c){var b=A(a);c&&g.extendOwn(b,c);return b};g.clone=function(a){return!g.isObject(a)?a:g.isArray(a)?a.slice():g.extend({},a)};g.tap=function(a,c){c(a);return a};g.isMatch=function(a,c){var b=g.keys(c),d=b.length;if(null==a)return!d;for(var e=Object(a),f=0;f<d;f++){var h=
...
g.isEqual=function(a,c){return K(a,c)};g.isEmpty=function(a){return null==a?!0:y(a)&&(g.isArray(a)||g.isString(a)||g.isArguments(a))?0===a.length:0===g.keys(a).length};g.isElement=function(a){return!!(a&&1===a.nodeType)};g.isArray=p||function(a){return"[object Array]"===k.call(a)};g.isObject=function(a){var c=typeof a;return"function"===c||"object"===c&&!!a};g.each("Arguments Function String Number Date RegExp Error".split(" "),function(a){g["is"+a]=function(c){return k.call(c)==="[object "+a+"]"}});
...
q.call(a,c)};g.noConflict=function(){e._=f;return this};g.identity=function(a){return a};g.constant=function(a){return function(){return a}};g.noop=function(){};g.property=w;g.propertyOf=function(a){return null==a?function(){}:function(c){return a[c]}};g.matcher=g.matches=function(a){a=g.extendOwn({},a);return function(c){return g.isMatch(c,a)}};g.times=function(a,c,b){for(var d=Array(Math.max(0,a)),c=D(c,b,1),b=0;b<a;b++)d[b]=c(b);return d};g.random=function(a,c){null==c&&(c=a,a=0);return a+Math.floor(Math.random()*
(c-a+1))};g.now=Date.now||function(){return(new Date).getTime()};p={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;","`":"&#x60;"};l=g.invert(p);w=function(a){var c=function(c){return a[c]},b="(?:"+g.keys(a).join("|")+")",d=RegExp(b),e=RegExp(b,"g");return function(a){a=null==a?"":""+a;return d.test(a)?a.replace(e,c):a}};g.escape=w(p);g.unescape=w(l);g.result=function(a,c,b){c=null==a?void 0:a[c];void 0===c&&(c=b);return g.isFunction(c)?c.call(a):c};var U=0;g.uniqueId=function(a){var c=
++U+"";return a?a+c:c};g.templateSettings={evaluate:/<%([\s\S]+?)%>/g,interpolate:/<%=([\s\S]+?)%>/g,escape:/<%-([\s\S]+?)%>/g};var J=/(.)^/,S={"'":"'","\\":"\\","\r":"r","\n":"n","\u2028":"u2028","\u2029":"u2029"},R=/\\|'|\r|\n|\u2028|\u2029/g,r=function(a){return"\\"+S[a]};g.template=function(a,c,b){!c&&b&&(c=b);var c=g.defaults({},c,g.templateSettings),b=RegExp([(c.escape||J).source,(c.interpolate||J).source,(c.evaluate||J).source].join("|")+"|$","g"),d=0,e="__p+='";a.replace(b,function(c,b,f,
g,h){e+=a.slice(d,h).replace(R,r);d=h+c.length;b?e+="'+\n((__t=("+b+"))==null?'':_.escape(__t))+\n'":f?e+="'+\n((__t=("+f+"))==null?'':__t)+\n'":g&&(e+="';\n"+g+"\n__p+='");return c});e+="';\n";c.variable||(e="with(obj||{}){\n"+e+"}\n");e="var __t,__p='',__j=Array.prototype.join,print=function(){__p+=__j.call(arguments,'');};\n"+e+"return __p;\n";try{var f=new Function(c.variable||"obj","_",e)}catch(h){throw h.source=e,h;}b=function(a){return f.call(this,a,g)};b.source="function("+(c.variable||"obj")+
...
delete b[0];return L(this,b)}});g.each(["concat","join","slice"],function(a){var c=h[a];g.prototype[a]=function(){return L(this,c.apply(this._wrapped,arguments))}});g.prototype.value=function(){return this._wrapped};g.prototype.valueOf=g.prototype.toJSON=g.prototype.value;g.prototype.toString=function(){return""+this._wrapped};"function"===typeof define&&define.amd&&define("underscore",[],function(){return g})}).call(this);var JSON;JSON||(JSON={});
(function(){function a(a){return 10>a?"0"+a:a}function c(a){e.lastIndex=0;return e.test(a)?'"'+a.replace(e,function(a){var c=j[a];return"string"===typeof c?c:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+a+'"'}function b(a,d){var e,j,s,m,u=f,t,g=d[a];g&&("object"===typeof g&&"function"===typeof g.toJSON)&&(g=g.toJSON(a));"function"===typeof i&&(g=i.call(d,a,g));switch(typeof g){case "string":return c(g);case "number":return isFinite(g)?String(g):"null";case "boolean":case "null":return String(g);case "object":if(!g)return"null";
f+=h;t=[];if("[object Array]"===Object.prototype.toString.apply(g)){m=g.length;for(e=0;e<m;e+=1)t[e]=b(e,g)||"null";s=0===t.length?"[]":f?"[\n"+f+t.join(",\n"+f)+"\n"+u+"]":"["+t.join(",")+"]";f=u;return s}if(i&&"object"===typeof i){m=i.length;for(e=0;e<m;e+=1)"string"===typeof i[e]&&(j=i[e],(s=b(j,g))&&t.push(c(j)+(f?": ":":")+s))}else for(j in g)Object.prototype.hasOwnProperty.call(g,j)&&(s=b(j,g))&&t.push(c(j)+(f?": ":":")+s);s=0===t.length?"{}":f?"{\n"+f+t.join(",\n"+f)+"\n"+u+"}":"{"+t.join(",")+
...
e=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,f,h,j={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},i;"function"!==typeof JSON.stringify&&(JSON.stringify=function(a,c,d){var e;h=f="";if("number"===typeof d)for(e=0;e<d;e+=1)h+=" ";else"string"===typeof d&&(h=d);if((i=c)&&"function"!==typeof c&&("object"!==typeof c||"number"!==typeof c.length))throw Error("JSON.stringify");return b("",{"":a})});
...
function getBrowserObject(a,c){var b=0,d;document.getElementById?d=document.getElementById(a):document.all?d=document.all[a]:document.layers&&(d=document.layers[a],b=1);c&&!b&&(d=d.style);return d}function loadUrl(a){document.location.href=a}function createCookie(a,c,b){if(b){var d=new Date;d.setTime(d.getTime()+864E5*b);b="; expires="+d.toGMTString()}else b="";document.cookie=a+"="+c+b+"; path=/"}
function readCookie(a){for(var a=a+"=",c=document.cookie.split(";"),b=0;b<c.length;b++){for(var d=c[b];" "==d.charAt(0);)d=d.substring(1,d.length);if(0==d.indexOf(a))return d.substring(a.length,d.length)}return null}function makeAsyncRequest(){return window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP")}function sendAsyncRequest(a,c,b,d){a.open(d,c,!0);a.setRequestHeader("Content-Type","text/html; Charset=utf-8");a.send(b)}
function changeFormAction(a,c){document.getElementById(a).action=c;document.getElementById(a).submit()}function urlEncode(a){return encodeURIComponent(a).replace(/\%20/g,"+").replace(/!/g,"%21").replace(/'/g,"%27").replace(/\(/g,"%28").replace(/\)/g,"%29").replace(/\*/g,"%2A").replace(/\~/g,"%7E")}function escapeHTML(a){return $("<div/>").text(a).html()}function unescapeHTML(a){return $("<div/>").html(a).text()};(function(a){a.pkp=a.pkp||{};a.pkp.classes=a.pkp.classes||{};a.pkp.controllers=a.pkp.controllers||{};a.pkp.plugins=a.pkp.plugins||{};a.pkp.plugins.blocks=a.pkp.plugins.blocks||{};a.pkp.plugins.generic=a.pkp.plugins.generic||{};a.pkp.plugins.pubIds=a.pkp.plugins.pubIds||{};a.pkp.plugins.importexport=a.pkp.plugins.importexport||{};a.pkp.classes.Helper=function(){throw Error("Trying to instantiate the Helper singleton!");};a.pkp.classes.Helper.CHARS_="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".split("");
...
this.$linkActionElement.focus();this.$modal_.remove();return this.parent("finish")}})(jQuery);(function(a){a.pkp.classes.notification=a.pkp.classes.notification||{};a.pkp.classes.notification.NotificationHelper=function(){};a.pkp.classes.notification.NotificationHelper.redirectNotifyUserEvent=function(c,b){var d,e,f,h,j,i,n,k,q,p,s;if(void 0!==b.content)c.getHtmlElement().parent().trigger("notifyUser",[b]);else{d=c.getHtmlElement();e=!1;!(c instanceof a.pkp.controllers.SiteHandler)&&0<a(b).parents(".pkp_controllers_grid").length&&(d.parent().trigger("notifyUser"),e=!0);d=a(".pkp_notification",
d);f=[];h=0;for(j=d.length;h<j;h++)if(n=a(d[h]),!(0<n.parents(":hidden").length)){i=n.parents();q=0;for(p=i.length;q<p;q++)if(k=a(i[q]).data("pkp.handler"),k instanceof a.pkp.classes.Handler){s=a(i[q]);break}if(s.has(b[0]).length||s[0]===b[0]){if(0<n.parents(".ui-accordion:first").length&&(i=n.parents(".ui-accordion:first"),!i.has(b[0])))continue;f.push(n)}}if(f.length)for(h=f.length-1;-1<h;h--)f[h].triggerHandler("notifyUser");else e||c.getHtmlElement().parent().trigger("notifyUser")}}})(jQuery);(function(a){a.pkp.classes.features=a.pkp.classes.features||{};a.pkp.classes.features.Feature=function(a,b){this.gridHandler=a;this.options_=b;this.addFeatureHtml(this.getGridHtmlElement(),b)};a.pkp.classes.features.Feature.prototype.gridHandler=null;a.pkp.classes.features.Feature.prototype.options_=null;a.pkp.classes.features.Feature.prototype.setOptions=function(a){this.options_=a};a.pkp.classes.features.Feature.prototype.getOptions=function(){return this.options_};a.pkp.classes.features.Feature.prototype.init=
...
function(a){a.children().after(this.getSequenceInput_());a=this.gridHandler.getRows();this.storeOrder(a)}})(jQuery);(function(a){a.pkp.classes.features.OrderMultipleListsItemsFeature=function(a,b){this.parent(a,b)};a.pkp.classes.Helper.inherits(a.pkp.classes.features.OrderMultipleListsItemsFeature,a.pkp.classes.features.OrderListbuilderItemsFeature);a.pkp.classes.features.OrderMultipleListsItemsFeature.prototype.addFeatureHtml=function(c,b){var d,e,f,h,j,i,n;this.parent("addFeatureHtml",c,b);d=a('<input type="hidden" name="newRowId[listId]" class="itemList" />');e=this.gridHandler.getRows();f=0;for(h=e.length;f<
h;f++)j=a(e[f]),i=this.gridHandler.getListIdByRow(j),n=d.clone(),n.attr("value",i),a("td.first_column",j).append(n)};a.pkp.classes.features.OrderMultipleListsItemsFeature.prototype.storeRowOrder=function(a,b){var d,e;this.parent("storeRowOrder",a,b);d=b.find(".itemList");e=this.gridHandler.getListIdByRow(b);d.attr("value",e)};a.pkp.classes.features.OrderMultipleListsItemsFeature.prototype.setupSortablePlugin=function(){var a=this.gridHandler.getLists().find("tbody");this.applySortPlgOnElements(a,
...
function(a,b){var d,e;d=this.getHtmlElement().find("th").length;e=a.first().find("td");e=b?e.attr("colspan"):e.length;return d==e};a.pkp.controllers.grid.GridHandler.prototype.replaceElementResponseHandler=function(c,b){var d,e,f,h,j,i,n;e=this.handleJson(b);if(!1!==e)if(e.elementNotFound)d=e.elementNotFound,d=this.getRowByDataId(d),0<d.length&&this.deleteElement(d);else{f=a(e.content);if(1<f.filter("tr:not('.row_controls')").length){d=f.filter("tr.gridRow");j=f.filter("tr.row_controls");i=0;for(n=
...
null),"json")):this.enableControls(),this.callFeaturesHook("addElement",d));return!1};a.pkp.controllers.listbuilder.ListbuilderHandler.prototype.fetchOptionsResponseHandler_=function(c,b){var d=this.handleJson(b),e=this.getHtmlElement(),f=[],h,j,i,n,k,q,p,s,m,u;if(!1!==d){e.find(".gridCellDisplay :input").each(function(c,b){f[c]=a(b).val()});e=e.find(".gridRowEdit:visible .selectMenu:input");h=0;for(j=e.length;h<j;h++){i=a(e[h]);n=i.parents(".gridCellContainer");k=0;i.children().empty();q=null;for(q in d.content[h])if(q!=
...
{title:"",okButton:this.blindReviewerWarningOk_,cancelButton:!1,dialogText:this.blindReviewerWarning_},a('<div id="'+a.pkp.classes.Helper.uuid()+'" class="pkp_modal pkpModalWrapper" tabindex="-1"></div>').pkpHandler("$.pkp.controllers.modal.ConfirmationModalHandler",c)};a.pkp.controllers.grid.users.stageParticipant.form.StageParticipantNotifyHandler.prototype.handleResponse=function(c,b){a.pkp.classes.Handler.getHandler(a("#queriesGrid .pkp_controllers_grid")).trigger("dataChanged");return this.parent("handleResponse",
c,b)}})(jQuery);(function(a){a.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler=function(c,b){this.parent(c,b);a("select[name^='filterUserGroupId']",c).change(this.callbackWrapper(this.addUserGroupId));a("input[name='userId']").click(function(){var c=a("input[name='userId']:checked").val();a("input[name='userIdSelected']").val(c).trigger("change")});this.addUserGroupId()};a.pkp.classes.Helper.inherits(a.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler,a.pkp.controllers.form.ClientFormHandler);
...
"json"));d.attr("id").match(/approvedProofGrid/)&&this.trigger("dataChanged")};a.pkp.controllers.tab.catalogEntry.CatalogEntryTabHandler.prototype.updateTabsHandler_=function(a,b){var d=this.handleJson(b),e=this.getHtmlElement(),f=e.find("li a"),h={},j=/publication(\d+)/,i,n,k;for(n=0;n<f.length;n++)k=f[n].getAttribute("id"),k=j.exec(k),null!==k&&(h[k[1]]=n);for(i in d.formats)h.hasOwnProperty(i)||(f=this.tabContentUrl_+"&representationId="+encodeURIComponent(i),f=f.replace(/[$]/g,"$$$$"),e.tabs("add",
...
function(c,b,d){var e=this.getHtmlElement();a.get(d.tabsUrl,function(a){e.prev("div").remove();e.replaceWith(a.content)})}})(jQuery);(function(a){a.pkp.controllers.catalog=a.pkp.controllers.catalog||{form:{}};a.pkp.controllers.catalog.form.CatalogMetadataFormHandler=function(c,b){this.parent(c,b);b.workTypeEditedVolume&&(this.workTypeEditedVolume_=b.workTypeEditedVolume);b.workTypeAuthoredWork&&(this.workTypeAuthoredWork_=b.workTypeAuthoredWork);a("#workType",c).change(this.callbackWrapper(this.toggleVolumeEditors));a("#audienceRangeExact",c).change(this.callbackWrapper(this.ensureValidAudienceRanges_));b.arePermissionsAttached&&
c.find("#attachPermissions").prop("checked",!0);this.coverImageMessage_=b.coverImageMessage;a('input[id^="copyrightHolder-"]',c).keyup(this.callbackWrapper(this.checkAttachMetadata));a('input[id^="copyrightYear-"]',c).keyup(this.callbackWrapper(this.checkAttachMetadata));a('input[id^="licenseURL-"]',c).keyup(this.callbackWrapper(this.checkAttachMetadata));a('input[id="confirm"]',c).change(function(){this.checked&&a("#attachPermissions").prop("checked",!0)});this.bind("fileUploaded",this.callbackWrapper(this.handleCoverImageUpload))};
