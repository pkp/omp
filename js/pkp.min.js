...
"json"));d.attr("id").match(/approvedProofGrid/)&&this.trigger("dataChanged")};a.pkp.controllers.tab.catalogEntry.CatalogEntryTabHandler.prototype.updateTabsHandler_=function(a,b){var d=this.handleJson(b),e=this.getHtmlElement(),f=e.find("li a"),h={},j=/publication(\d+)/,i,l,k;for(l=0;l<f.length;l++)k=f[l].getAttribute("id"),k=j.exec(k),null!==k&&(h[k[1]]=l);for(i in d.formats)h.hasOwnProperty(i)||(f=this.tabContentUrl_+"&representationId="+encodeURIComponent(i),f=f.replace(/[$]/g,"$$$$"),e.tabs("add",
