...
class PayPalPlugin extends PaymethodPlugin {
	...
	function displayPaymentForm($queuedPaymentId, $queuedPayment, $request) {
		...
		$params = array(
			...
			'business' => $this->getSetting($press->getId(), 'selleraccount'),
					...
					case 'Completed':
						...
						if ($payPalDao->transactionExists($transactionId)) {
							...
							if (
								...
								($grantedEmail = $request->getUserVar('receiver_email')) != ($queuedEmail = $this->getSetting($press->getId(), 'selleraccount'))
							) {
