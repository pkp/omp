		...
		{foreach from=spotlights item=spotlight}
			...
			<li class="pkp_sidebar_spotlight" id="sidebarSpotlight{$spotlight->getId()|escape}">
				{if $spotlight>getAssocType() == 3} {* SPOTLIGHT_TYPE_BOOK *}
					<a href="{url op="book" path=$publishedMonograph->getId()}"><img src="{url router=$smarty.const.ROUTE_COMPONENT component="submission.CoverHandler" op="thumbnail" monographId=$item->getId()}" /></a>
