			...
			{foreach from=$spotlights item=spotlight}
				...
				<li class="pkp_sidebar_spotlight" id="sidebarSpotlight{$spotlight->getId()|escape}">
					{if $spotlight->getAssocType() == 3} {* SPOTLIGHT_TYPE_BOOK *}
						<a href="{url router=$smarty.const.ROUTE_PAGE page="catalog" op="book" path=$item->getId()}"><img src="{url router=$smarty.const.ROUTE_COMPONENT component="submission.CoverHandler" op="thumbnail" monographId=$item->getId()}" /></a>
						<div class="pkp_sidebar_spotlight_monographTitle pkp_helpers_clear">{$item->getLocalizedTitle()|strip_unsafe_html}</div>
						<div class="pkp_sidebar_spotlight_monographAbstract">{$item->getLocalizedAbstract()|strip_unsafe_html|truncate:80}</div>
					{/if}
				</li>
			{/foreach}
		</ul>
