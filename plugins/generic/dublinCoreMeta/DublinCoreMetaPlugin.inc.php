...
class DublinCoreMetaPlugin extends GenericPlugin {
	...
	function monographView($hookName, $args) {
		...
		if (($issn = $press->getSetting('onlineIssn')) || ($issn = $press->getSetting('printIssn')) || ($issn = $press->getSetting('issn'))) {
		...
		$i=0;
		if ($subjects = $monograph->getSubject(null)) foreach ($subjects as $locale => $localeSubject) {
