...
class DublinCoreMetaPlugin extends GenericPlugin
{
    ...
    /**
     * Inject Dublin Core metadata into the monograph view
     *
     * @param string $hookName
     * @param array $args
     */
    public function monographView($hookName, $args): bool
    {
        $request = $args[0];
        $monograph = $args[1];
        ...
        foreach ($titles as $locale => $altTitle) {
            if ($title != '' && $locale != $publicationLocale) {
                $templateMgr->addHeader('dublinCoreAltTitle' . $locale, '<meta name="DC.Title.Alternative" xml:lang="' . htmlspecialchars(LocaleConversion::toBcp47($locale)) . '" content="' . htmlspecialchars($altTitle) . '"/>');
            }
        }
    ...
    /**
     * Inject Dublin Core metadata into the monograph file view
     *
     * @param string $hookName
     * @param array $args
     *
    ...
    public function monographFileView($hookName, $args)
    {
        ...
        $templateMgr->addHeader('dublinCoreLanguage', '<meta name="DC.Language" scheme="ISO639-1" content="' . LocaleConversion::toBcp47($publicationLocale) . '"/>');
