...
<body class="pkp_page_{$requestedPage|escape} pkp_op_{$requestedOp|escape}">
	...
	<div id="htmlContainer" class="viewable_file_frame{if !$isLatestPublication} viewable_file_frame_with_notice{/if}" style="overflow:visible;-webkit-overflow-scrolling:touch">
		...
		{/if}
		<iframe name="htmlFrame" src="{$downloadUrl}" title="{translate key="submission.representationOfTitle" representation=$publicationFormat->getLocalizedName()|escape title=$filePublication->getLocalizedFullTitle()|escape}" allowfullscreen webkitallowfullscreen></iframe>
	</div>
