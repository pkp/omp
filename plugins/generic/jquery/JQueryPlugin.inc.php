 ...
 *
 * @brief Plugin to allow jQuery scripts to be added to OMP
 */
...
class JQueryPlugin extends GenericPlugin {
	...
	function getEnabledScripts($page, $op) {
		$scripts = array();
		/* switch ("$page/$op") {
			case 'manager/submissionCategory':
	...
	function manage($verb, $args, &$message) {
		...
		switch ($verb) {
			case 'enable':
				$this->updateSetting($pressId, 'enabled', true);
				$message = Locale::translate('plugins.generic.jquery.enabled');
				break;
			case 'disable':
				$this->updateSetting($pressId, 'enabled', false);
