...
description: OMP Playwright e2e suite — entry point. Loads the shared harness docs (lib/pkp/docs/e2e/dev/) and carries the OMP-specific deltas. Use when writing or debugging Playwright specs, POMs, fixtures, or scenario seeds in OMP.
---
...
- **Fleet**: port 8100, DB `omp_test`, project name `omp`. Context is a
  *press*; cross-app vocabulary in `lib/pkp/docs/product/APP-GLOSSARY.md`.
...
- **No issues** on OMP — the counterpart concept is the catalog
  (APP-GLOSSARY; absence is not a synonym).
