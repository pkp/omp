...
{url|assign:"currentUrl" page="seriesEditor"}
...
<form action="#">
<ul class="filter">
	<li>{translate key="editor.submissions.inSeries"}: <select name="filterSeries" onchange="location.href='{url|escape path=$pageToDisplay searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth filterCategory="SERIES_ID" escape=false}'.replace('SERIES_ID', this.options[this.selectedIndex].value)" size="1" class="selectMenu">{html_options options=$seriesOptions selected=$filterSeries}</select></li>
</ul>
</form>
