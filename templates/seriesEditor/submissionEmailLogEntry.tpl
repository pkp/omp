 ...
 *}
{strip}
{assign var="pageTitle" value="submission.history.submissionEmailLog"}
	...
	<tr valign="top">
		...
		<td class="value">
			{if $logEntry->getSenderFullName()}
				...
				{url|assign:"url" page="user" op="email" to=$emailString|to_array redirectUrl=$currentUrl subject=$logEntry->getSubject() monographId=$submission->getMonographId()}
	...
	{if !empty($attachments)}
		<tr valign="top">
			...
			<td class="value">{foreach from=$attachments item=attachment}
				<a href="{url op="downloadFile" path=$attachment->getMonographId()|to_array:$attachment->getFileId()}" class="action">{$attachment->getOriginalFilename()|escape}</a>
			{/foreach}</td>
		</tr>
	{/if}
	<tr valign="top">
...
{if $isEditor}
	<a href="{url op="clearSubmissionEmailLog" path=$submission->getMonographId()|to_array:$logEntry->getLogId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.email.confirmDeleteLogEntry"}')" class="action">{translate key="submission.email.deleteLogEntry"}</a><br/>
{/if}

<a href="{url op="submissionEmailLog" path=$submission->getMonographId()}" class="action">{translate key="submission.email.backToEmailLog"}</a>
