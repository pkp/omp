...
<form name="submit" method="post" action="{url op=$actionHandler path=$monographId}">
...
<p>{foreach from=$alphaList item=letter}<a href="{url op=$actionHandler path=$monographId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a> {/foreach}<a href="{url op=$actionHandler path=$monographId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>
