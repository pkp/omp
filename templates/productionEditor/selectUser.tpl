<!-- templates/productionEditor/selectUser.tpl -->
...
<form name="submit" method="post" action="{url op=$actionHandler path=$monographId|to_array:$productionAssignmentId}">
...
<p>{foreach from=$alphaList item=letter}<a href="{url op=$actionHandler path=$monographId|to_array:$productionAssignmentId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a> {/foreach}<a href="{url op=$actionHandler path=$monographId|to_array:$assignmentId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>
...
<tr valign="top">
	...
	<td>
		...
		{else}
			<a href="{url op=$actionHandler path=$monographId|to_array:$productionAssignmentId:$userid}" class="action">{translate key="common.assign"}</a>
		{/if}
	</td>
</tr>
