...
{foreach from=$monographComments item=comment}
<tr valign="top">
	<td width="25%">
	...
	<td width="75%">
		{if $comment->getAuthorId() eq $userId and not $isLocked}
			<div style="float: right"><a href="{if $reviewId}{url op="editComment" path=$monographId|to_array:$comment->getCommentId() reviewId=$reviewId}{else}{url op="editComment" path=$monographId|to_array:$comment->getCommentId()}{/if}" class="action">{translate key="common.edit"}</a> <a href="{if $reviewId}{url op="deleteComment" path=$monographId|to_array:$comment->getCommentId() reviewId=$reviewId}{else}{url op="deleteComment" path=$monographId|to_array:$comment->getCommentId()}{/if}" onclick="return confirm('{translate|escape:"jsparam" key="submission.comments.confirmDelete"}')" class="action">{translate key="common.delete"}</a></div>
		{/if}
