{**
 * templates/frontend/components/objectFiles.tpl
 *
 ...
 * @uses $objectFiles array if a chapter is requested files of the requested chapter, else non chapter files
...
{if $objectFiles|@count || (!$isChapterRequest && $remotePublicationFormats|@count)}
	<div class="item files">
		...
		{foreach from=$publicationFormats item=format}
			...
			{if $format->getRemoteUrl() && !$isChapterRequest}
				...
				{* File downloads *}
			{else}
				{pluck_files assign=pubFormatFiles files=$objectFiles by="publicationFormat" value=$format->getId()}
