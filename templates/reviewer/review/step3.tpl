...
{fbvFormArea id="reviewStep3"}
	{fbvFormSection label="common.download" description="reviewer.monograph.downloadDescription"}
		{url|assign:reviewFilesGridUrl router=$smarty.const.ROUTE_COMPONENT component="grid.files.review.ReviewerReviewFilesGridHandler" op="fetchGrid" monographId=$submission->getId() stageId=$reviewAssignment->getStageId() reviewRoundId=$reviewRoundId reviewAssignmentId=$reviewAssignment->getId() escape=false}
		{load_url_in_div id="reviewFiles" url=$reviewFilesGridUrl}
	{/fbvFormSection}

	{fbvFormSection label="submission.review" description="reviewer.monograph.reviewDescription"}
