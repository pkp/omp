...
<form class="pkp_form" id="addStageParticipant" method="post" action="#">
	{include file="common/formErrors.tpl"}

	<p>{translate key="submission.submit.addStageParticipant.description"}</p>

	<span style="padding-left:10px;">{fbvElement type="select" name="userGroupId" id="userGroupId" from=$userGroupOptions translate=false}</span>
	...
	{fbvFormArea id="buttons"}
	    {fbvFormSection}
	        {fbvElement type="submit" id="submitFormButton" label="common.ok"}
	    {/fbvFormSection}
	{/fbvFormArea}
</form>