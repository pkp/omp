...
<div id="newCatalogEntryTabs" class="pkp_controllers_tab">
	<ul>
		...
		{assign var=tabsCount value=3}
		{if $citationsEnabled}
			<li>
				<a name="citations" href="{url router=$smarty.const.ROUTE_COMPONENT component="tab.catalogEntry.CatalogEntryTabHandler" tab="citations" op="citations" submissionId=$submissionId stageId=$stageId tabPos="3"}">{translate key="submission.citations"}</a>
			</li>
			{assign var=tabsCount value=4}
		{/if}
		{counter start=$tabsCount assign="counter"}
