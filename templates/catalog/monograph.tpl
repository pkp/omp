...
<li class="pkp_catalog_monograph {if $inline}pkp_helpers_align_left{/if} pkp_helpers_text_center">
	...
	<div class="pkp_catalog_monographDetails">
		...
		<div class="pkp_catalog_monographTitle"><a href="{url router=$smarty.const.ROUTE_PAGE page="catalog" op="book" path=$publishedMonograph->getId()}">{$monographTitle}</a></div>
		{if $publishedMonograph->getLocalizedSubtitle() != ''}
			<div class="pkp_catalog_monographSubtitle">{$publishedMonograph->getLocalizedSubtitle()}</div>
		{/if}
	...
	<div class="pkp_catalog_monographFormats">
		{foreach from=$publishedMonograph->getCatalogFormatInfo() item="details"}
			<div class="pkp_catalog_monographFormat">{$details.name|escape}: {if $details.price != 0}{$details.price} ({$pressCurrency}){else}{translate key="payment.directSales.openAccess"}{/if}</div>
