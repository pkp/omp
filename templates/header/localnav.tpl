	...
	{if $currentPress}
		...
		<li><a href="#">{translate key="navigation.about"}</a>
			<ul>
				...
				{if not ($currentPress->getLocalizedSetting('contributorNote') == '' && empty($contextSettings.contributors) && $currentPress->getLocalizedSetting('sponsorNote') == '' && empty($contextSettings.sponsors))}<li><a href="{url router=$smarty.const.ROUTE_PAGE page="about" op="sponsorship"}">{translate key="about.pressSponsorship"}</a></li>{/if}
			</ul>
		</li>
	{/if}
{/capture}
