...
<li class="pkp_manageCatalog_monograph monograph_id_{$monographId|escape}{if !$isFeatured} not_sortable{/if}" id="{$monographContainerId|escape}">
	<div class="pkp_manageCatalog_monograph_image">
		...
		<img height="{$coverImage.thumbnailHeight}" width="{$coverImage.thumbnailWidth}" src="{url router=$smarty.const.ROUTE_COMPONENT component="submission.CoverHandler" op="thumbnail" monographId=$monograph->getId()}" alt="{$monograph->getLocalizedTitle()|escape}" />
	</div>
	<div class="pkp_manageCatalog_monograph_actions pkp_linkActions">
		{fbvFormSection list="true"}
			<li>{null_link_action key="submission.editCatalogEntry" id="catalogEntry-"|concat:$monographId image="information"}</li>
			<li>{null_link_action key="submission.goToWorkflow" id="workflow-"|concat:$monographId image="revisions"}</li>
		{/fbvFormSection}
	</div>
