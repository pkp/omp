 ...
 *
 * Acquisitions editor index.
 *
 * $Id$
 *}
{strip}
{assign var="pageTitle" value="common.queue.long.$pageToDisplay"}
{url|assign:"currentUrl" page="acquisitionsEditor"}
...
<form action="#">
<ul class="filter">
	<li>{translate key="editor.submissions.inArrangement"}: <select name="filterArrangement" onchange="location.href='{url|escape path=$pageToDisplay searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth filterCategory="ARRANGEMENT_ID" escape=false}'.replace('ARRANGEMENT_ID', this.options[this.selectedIndex].value)" size="1" class="selectMenu">{html_options options=$arrangementOptions selected=$filterArrangement}</select></li>
</ul>
</form>
...
{include file="acquisitionsEditor/$pageToDisplay.tpl"}
