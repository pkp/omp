...
<ul class="menu">
	...
	<li class="current"><a href="{url op="submissionEmailLog" path=$submission->getMonographId()}">{translate key="submission.history.submissionEmailLog"}</a></li>
</ul>
...
{if $isEditor}
<a class="action" href="{url op="clearSubmissionEmailLog" path=$submission->getMonographId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.email.confirmClearLog"}')">{translate key="submission.history.clearLog"}</a>
{/if}
</div>
