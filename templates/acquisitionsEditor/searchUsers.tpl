...
<form name="submit" method="post" action="{url op="enrollSearch" path=$monographId}">
...
<p>{foreach from=$alphaList item=letter}<a href="{url op="enrollSearch" path=$monographId searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a> {/foreach}<a href="{url op="enrollSearch" path=$monographId}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></p>

<div id="users">
<form action="{url op="enroll" path=$monographId}" method="post">
...
<tr valign="top">
	...
	<td><a href="{url op="enroll" path=$monographId userId=$user->getUserId()}" class="action">{translate key="manager.people.enroll"}</a></td>
</tr>
